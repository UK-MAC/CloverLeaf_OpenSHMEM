...
SHMEM_CRAY = -DENABLE_SHMEM_FINALIZE
SHMEM_INTEL = 
SHMEM_SUN =
SHMEM_GNU =
SHMEM_PGI =
...
FLAGS=$(FLAGS_$(COMPILER)) $(OMP) $(I3E) $(OPTIONS) $(SHMEM_FENCE) $(SHMEM_FINALIZE) $(SHMEM_INCLUDE) $(SHMEM_LIB) $(PUTS)
...
clean:
	rm -f *.o *.mod *genmod* *cuda* *hmd* *.cu *.oo *.hmf *.lst *.cub *.ptx *.cl *.i clover_leaf
